...
describe('GET /api/gtfs/city-vehicles', () => {
  beforeAll(() => {
    positions = []
  ...
  it('rejects an unknown city with 400 and no echo of the value', async () => {
    const { response, body } = await call('http://localhost/api/gtfs/city-vehicles?city=zzz')
    expect(response.status).toBe(400)
    expect(JSON.stringify(body)).not.toContain('zzz')
  })

  it('rejects a malformed city with 400', async () => {
    const { response } = await call('http://localhost/api/gtfs/city-vehicles?city=..%2Fx')
    expect(response.status).toBe(400)
  })

  it('returns [] with state loading when the vehicle poller is not ready', async () => {
    vehiclePollerReady = false
    const { response, body } = await call('http://localhost/api/gtfs/city-vehicles?city=warszawa')
    vehiclePollerReady = true
  ...
  it('returns every position, matched or not, no delay field', async () => {
    positions = [
      { id: 'V/1', tripId: 'T', lat: 52.21, lon: 21.0, sideNumber: '1', bearing: null, timestamp: new Date().toISOString() },
      { id: 'V/2', tripId: 'UNKNOWN', lat: 52.3, lon: 21.1, sideNumber: '9', bearing: null, timestamp: new Date().toISOString() },
    ]
    const { response, body } = await call('http://localhost/api/gtfs/city-vehicles?city=warszawa')
    expect(response.status).toBe(200)
